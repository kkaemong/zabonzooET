...
public class StageCardUI : MonoBehaviour, IPointerEnterHandler, IPointerClickHandler
{
    ...
    public string SceneName => !string.IsNullOrWhiteSpace(sceneName) ? sceneName : stageInfo != null ? stageInfo.SceneName : string.Empty;
    ...
    private void Awake()
    {
        ...
        if (thumbnailImage == null)
        {
            ...
            WarnAutoAssigned("thumbnailImage", thumbnailImage);
        }

        if (lockIcon == null)
        {
            lockIcon = FindImage("LockIcon");
            WarnAutoAssigned("lockIcon", lockIcon);
        }

        if (titleText == null)
        {
            titleText = FindText("TitleText");
            WarnAutoAssigned("titleText", titleText);
        }

        if (descriptionText == null)
        {
            descriptionText = FindText("DescriptionText");
            WarnAutoAssigned("descriptionText", descriptionText);
        }

        if (stateText == null)
        {
            stateText = FindText("StateText");
            WarnAutoAssigned("stateText", stateText);
        }
        ...
        if (outlineTargetGraphic != null)
        {
            selectionOutline = outlineTargetGraphic.GetComponent<Outline>();

            if (selectionOutline == null)
            {
                selectionOutline = outlineTargetGraphic.gameObject.AddComponent<Outline>();
            }
    ...
    public void RefreshContent()
    {
        ...
        Debug.Log(
            $"StageCardUI.RefreshContent: card='{name}', stageInfo='{stageInfo.name}', " +
            $"stageName='{stageInfo.StageName}', isUnlocked={IsUnlocked()}, " +
            $"thumbnail='{(stageInfo.Thumbnail != null ? stageInfo.Thumbnail.name : "null")}'",
            this);
    ...
    private bool IsUnlocked()
    {
        ...
        return UserDataManager.Instance.IsStageUnlocked(stageInfo);
    }

    private void WarnAutoAssigned(string fieldName, Object target)
    {
        if (target != null)
        {
            Debug.LogWarning($"StageCardUI.Awake: '{name}' auto-assigned '{fieldName}'. Assign it in the inspector if this should be explicit.", this);
        }
    }
}
