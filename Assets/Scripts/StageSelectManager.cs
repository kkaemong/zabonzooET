...
public class StageSelectManager : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        SelectCard(Mathf.Clamp(defaultSelectedIndex, 0, stageCards.Count - 1));
    }
    ...
    public void StartSelectedStage()
    {
        ...
        if (UserDataManager.Instance != null && !UserDataManager.Instance.IsStageUnlocked(stageInfo))
        {
            Debug.LogWarning($"StageSelectManager: Stage '{stageInfo.StageId}' is locked.", this);
            return;
        }

        string sceneName = selectedCard.SceneName;
    ...
    private void InitializeCards()
    {
        for (int i = 0; i < stageCards.Count; i++)
        {
            ...
            StageInfo stageInfo = card.StageInfo;
            Debug.Log(
                $"StageSelectManager.InitializeCards: index={i}, card='{card.name}', " +
                $"stageInfo='{(stageInfo != null ? stageInfo.name : "null")}'",
                this);
    ...
    private void MoveSelection(int direction)
    {
        ...
        if (nextIndex < 0)
        {
            nextIndex = Mathf.Clamp(defaultSelectedIndex, 0, stageCards.Count - 1);
        }
        else
        {
            nextIndex += direction;
